...
const templateCards: {
...
}[] = [
  {
    ...
    subtitle: "Documents → Manager approval → IT provisioning",
    badge: "Best demo",
  },
  {
    key: "leave-approval",
    title: "Leave Approval",
    subtitle: "Request → Policy check → Approval routing",
    badge: "Fast build",
  },
  {
    key: "document-verification",
    title: "Document Verification",
    subtitle: "Upload → Review → Auto-generate summary",
  ...
  switch (kind) {
    case "start":
      return {
        kind,
        title: "New Start",
        subtitle: "Entry point for workflow",
        metaLabel: "Template",
        metaValue: "Custom",
        accent: "emerald",
      };
    ...
    case "approval":
      return {
        kind,
        title: "New Approval",
        subtitle: "Approval decision step",
        metaLabel: "Role",
        metaValue: "Manager",
        accent: "amber",
      };
    case "automated": {
      ...
      return {
        ...
        subtitle: defaultAction?.description ?? "System action step",
        metaLabel: "Action",
        metaValue: defaultAction?.label ?? "Automation",
        accent: "violet",
        actionId: defaultAction?.id,
        params: Object.fromEntries(
          (defaultAction?.paramKeys ?? []).map((key) => [key, ""])
        ),
      };
    }
    case "end":
      return {
        kind,
        title: "New End",
        subtitle: "Workflow completion step",
        metaLabel: "Status",
        metaValue: "Done",
        accent: "slate",
      };
...
export function AppShell() {
  ...
  const loadTemplate = (templateKey: WorkflowTemplateKey) => {
    const flow = cloneTemplateFlow(templateKey);
    setFlowNodes(flow.nodes);
    setFlowEdges(flow.edges);
    setFlowStats(flow.stats);
      ...
      prev.map((node) =>
        node.id === nodeId
          ? {
              ...
              data: {
                ...node.data,
                title: patch.title ?? node.data.title,
                subtitle: patch.subtitle ?? node.data.subtitle,
                metaLabel: patch.metaLabel ?? node.data.metaLabel,
                metaValue: patch.metaValue ?? node.data.metaValue,
                actionId: patch.actionId ?? node.data.actionId,
                params: patch.params ?? node.data.params,
              },
            }
          : node
      )
  ...
  const handleRunSimulation = async () => {
    ...
    const result = await simulateWorkflowApi(flowNodes, activeTemplate);
  ...
  const workflowPayload = useMemo(
    () => ({
      ...
      nodes: flowNodes.map((node) => ({
        ...
        metaLabel: node.data.metaLabel,
        metaValue: node.data.metaValue,
      ...
      <div className="flex min-h-screen flex-col">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[250px_minmax(0,1fr)]">
          <aside className="border-r border-white/60 bg-white/60 p-5 backdrop-blur-xl">
            ...
            <div className="mt-5 rounded-3xl border border-white/70 bg-white/80 p-4 shadow-[0_12px_40px_rgba(15,23,42,0.06)]">
              ...
              <div className="mt-4 rounded-2xl border border-dashed border-violet-200 bg-violet-50 p-3 text-xs text-violet-700">
                Drag any node card from here and drop it directly onto the live workflow canvas.
              </div>
            </div>
          </aside>
          ...
          <main className="flex min-h-0 flex-col p-5">
            <div className="rounded-3xl border border-white/70 bg-white/85 p-4 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur-xl">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Drag nodes in, connect them, and configure live automation steps.
                  </p>
                </div>
            ...
            <div className="mt-5 grid items-start gap-5 xl:grid-cols-4">
              ...
              <div className="rounded-3xl border border-white/70 bg-white/85 p-4 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      Serialize the graph and ship the current state
                    </p>
                  </div>
                ...
                <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-950 p-3 shadow-inner">
                  <pre className="max-h-[340px] overflow-auto whitespace-pre-wrap text-[11px] leading-5 text-emerald-200">
{workflowJson}
                  </pre>
                </div>
              </div>
            ...
            <div className="mt-5 rounded-3xl border border-white/70 bg-white/85 p-5 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
              <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
              ...
              {!simulationSummary && !isRunningSimulation && (
                <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-600">
                  Run the simulation to generate a real execution trace for the current workflow.
                  Your edited titles, automation actions, and parameter choices will appear in the output.
                </div>
              )}
